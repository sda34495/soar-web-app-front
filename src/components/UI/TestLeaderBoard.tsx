...
const LeaderboardTable = () => {
  ...
  useEffect(() => {
    const fetchLeaderboardData = async () => {
      try {
        setLoading(true);
        const response = await getData("leaderboard/all"); // Adjust endpoint if necessary

        if (response?.data?.success) {
          setLeaderboardData(response.data.data); // Assuming the data is in response.data.data
        } else {
                ...
                .map((entry: any, index: number) => (
                  <tr key={index} className="hover:bg-gray-800 transition-all">
                    ...
                    <td className="p-3 flex items-center space-x-3">
                      <img
                        src={entry.profile_url || "/default-avatar.png"} // Provide a default image if not available
                        alt={entry.user}
