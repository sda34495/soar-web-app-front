  ...
  const handleCheckboxChange = async (
    ...
    try {
      ...
      if (response?.data?.success) {
        fetchCheckInDetails();
        toast.success("Substance Check-ins updated successfully", {
          id: loadingToast,
        });
      }
    } catch (error) {
    ...
    >
      <div className="bg-[#121212] text-white rounded-2xl shadow-md p-6 space-y-4 ">
        ...
        <div className="flex justify-between items-center ">
          ...
          <div className={` font-semibold ${hideText && "hidden"}`}>
              <span className={" hidden md:block  text-red-500 "}>{text}</span>
            </div>
